...
from scripts.conditioning import ConditionGenerator
...
class SynthSARDataset(Dataset):
    def __init__(self, dataset_path, train=False, num_channels=1, crop_size=(256, 256),
                 length=-1, seed=None, conditioning_args=None):
        """
        conditioning_args: optional args/config object forwarded to
            ConditionGenerator (enable_multilook, look_min, look_max,
            structure_kernels, wavelet_type, etc). See conditioning.py.
            Pass None to use ConditionGenerator's defaults (all
        ...
        # ConditionGenerator instantiated ONCE — every subsequent
        # __getitem__ call reuses it. It owns its own RNG (seeded off the
        # dataset seed, deterministic per-worker after _load_rng) so
        # look_num sampling stays reproducible.
        condition_rng = np.random.default_rng(seed)
        self.conditioner = ConditionGenerator(args=conditioning_args, rng=condition_rng)
    ...
    def __getitem__(self, idx):
        ...
        # All conditioning tensors are generated from clean_array — the
        # same normalized array used below to build clean_tensor — in a
        # single call, before any channel-repeat happens. This is the
        # ONLY place conditioning is computed.
        conditions = self.conditioner.generate(clean_array)
...
class SARDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hmx1, hmy1, hmx2, hmy2 = map(int, image_data[1:5])
        htx1, hty1, htx2, hty2 = map(int, image_data[5:9])
